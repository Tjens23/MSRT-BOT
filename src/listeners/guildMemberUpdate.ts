...
import { updateServerStats } from '../utils/serverStats';
...
export class GuildMemberUpdateListener extends Listener {
	...
	public async run(oldMember: GuildMember, newMember: GuildMember): Promise<void> {
		...
		if (oldMember.roles.cache.size !== newMember.roles.cache.size || !oldMember.roles.cache.every((role) => newMember.roles.cache.has(role.id))) {
			...
			// Update server stats channels when roles change
