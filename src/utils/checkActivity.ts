import { CategoryChannel, TextChannel, ButtonBuilder, ButtonStyle, ActionRowBuilder, EmbedBuilder, ComponentType } from 'discord.js';
...
export const CheckActivity = async () => {
	...
	if (!guild) return console.log("Couldn't find guild");
	...
	for (const member of Array.from(members.values())) {
		...
		if (lastActivity === 0 || lastActivity < twoWeeksAgo) {
			try {
				...
				// Create buttons
				const yesButton = new ButtonBuilder().setCustomId(`kick_${member.id}`).setLabel('✅ Yes, Kick').setStyle(ButtonStyle.Danger);

				const noButton = new ButtonBuilder().setCustomId(`no_kick_${member.id}`).setLabel('❌ No, Keep').setStyle(ButtonStyle.Success);
